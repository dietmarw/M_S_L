...
package Modelica_Mechanics_MultiBody_Interfaces
  ...
  connector Frame
    ...
    Modelica_Mechanics_MultiBody.Frames.Orientation R
...
Joints.Revolute.");
    flow Modelica_SIunits.Torque t[3] "Cut-torque resolved in connector frame";
...
object and an assert to check that both frame connectors are connected.
  ...
  */
    r_rel_b = Modelica_Mechanics_MultiBody.Frames.resolve2(frame_b.R, frame_b.r_0
       - frame_a.r_0);

    /* Force and torque balance between frame_a and frame_b */
    zeros(3) = frame_a.f + Modelica_Mechanics_MultiBody.Frames.resolveRelative(
        frame_b.f,
        frame_b.R,
        frame_a.R);
    zeros(3) = frame_a.t + Modelica_Mechanics_MultiBody.Frames.resolveRelative(
        frame_b.t + cross(r_rel_b, frame_b.f),
        frame_b.R,
        frame_a.R);
    annotation (Documentation(info="<HTML>
<p>
...
to which this force element is connected.
  ...
  equation
    ...
    frame_a.f = -e_a*f;
    frame_b.f = -Modelica_Mechanics_MultiBody.Frames.resolve2(
      Modelica_Mechanics_MultiBody.Frames.relativeRotation(frame_a.R, frame_b.R),
      frame_a.f);

    // Additional equations, if direct connections of line forces
    if fixedRotationAtFrame_a then
      Connections.root(frame_a.R);
      frame_a.R = Modelica_Mechanics_MultiBody.Frames.nullRotation();
    else
      frame_a.t = zeros(3);
    end if;
    ...
    if fixedRotationAtFrame_b then
      ...
      frame_b.R = Modelica_Mechanics_MultiBody.Frames.nullRotation();
    else
      frame_b.t = zeros(3);
    end if;
...
with the blocks of package Modelica.Blocks.
  ...
  partial model PartialRelativeSensor
    ...
    extends Modelica_Icons.RotationalSensor;
    parameter Integer n_out = 1 "Number of output signals";
    Frame_a frame_a "Coordinate system a" annotation (Placement(transformation(
            extent={{-116,-16},{-84,16}}, rotation=0)));
    Frame_b frame_b "Coordinate system b" annotation (Placement(transformation(
            extent={{84,-16},{116,16}}, rotation=0)));
