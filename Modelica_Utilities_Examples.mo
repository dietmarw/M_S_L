...
<blockquote><pre>
  ...
  protected
    ...
    Integer nextIndex;
    Integer iline=1;
    Modelica_Utilities.Types.TokenValue token;
    String message = "in file \"" + fileName + "\" on line ";
    String message2;
    Boolean found = false;
    Boolean endOfFile=false;
  algorithm
    (line,endOfFile) := Modelica_Utilities.Streams.readLine(fileName, iline);

    while not found and not endOfFile loop
      (token,nextIndex) := Modelica_Utilities.Strings.scanToken(line);
      ...
      elseif token.tokenType == Modelica_Utilities.Types.TokenType.IdentifierToken
          ...
          (delimiter,nextIndex) := Modelica_Utilities.Strings.scanDelimiter(
              line,
              nextIndex,
              {"="},
              message2);
          ...
          (delimiter,nextIndex) := Modelica_Utilities.Strings.scanDelimiter(
              ...
              {";",""},
              message2);
          Modelica_Utilities.Strings.scanNoToken(
              line,
              nextIndex,
              message2);
             found := true;
          ...
          // wrong token
        Modelica_Utilities.Strings.syntaxError(
            line,
            nextIndex,
            "Expected identifier " + message + String(iline));
       end if;
    ...
    if not found then
      Modelica_Utilities.Streams.error("Parameter \"" + name +
...
readRealParameter(\"test.txt\", \"w_rel0\")
  ...
  model readRealParameterModel
    ...
    parameter String file = Modelica_Utilities.Files.loadResource(
