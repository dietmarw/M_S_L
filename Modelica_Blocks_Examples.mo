...
encapsulated package Modelica_Blocks_Examples
  ...
  model PID_Controller
    ...
    Modelica_Mechanics_Rotational.Sources.Torque torque annotation (Placement(
    ...
    Modelica_Mechanics_Rotational.Components.SpringDamper spring(
      ...
      w_rel(fixed=true)) annotation (Placement(transformation(extent={{32,-20},
              {52,0}}, rotation=0)));
    Modelica_Mechanics_Rotational.Components.Inertia inertia2(J=2) annotation (
        Placement(transformation(extent={{60,-20},{80,0}}, rotation=0)));
    Modelica_Blocks.Sources.KinematicPTP kinematicPTP(
        startTime=0.5,
        deltaq={driveAngle},
        qd_max={1},
        qdd_max={1}) annotation (Placement(transformation(extent={{-92,20},{-72,
                40}}, rotation=0)));
    Modelica_Blocks.Continuous.Integrator integrator(initType=Modelica_Blocks.Types.Init.InitialState)
        annotation (Placement(transformation(extent={{-63,20},{-43,40}},
              rotation=0)));
    Modelica_Mechanics_Rotational.Sensors.SpeedSensor speedSensor annotation (
        Placement(transformation(extent={{22,-50},{2,-30}}, rotation=0)));
    Modelica_Mechanics_Rotational.Sources.ConstantTorque loadTorque(
...
is forced back to its limit after a transient phase.
  ...
  model Filter "Demonstrates the Continuous.Filter block with various options"
    ...
    parameter Modelica_SIunits.Frequency f_cut=2;
    parameter Modelica_Blocks.Types.FilterType filterType=Modelica_Blocks.Types.FilterType.LowPass
      "Type of filter (LowPass/HighPass)";
    parameter Modelica_Blocks.Types.Init init=Modelica_Blocks.Types.Init.SteadyState
      "Type of initialization (no init/steady state/initial state/initial output)";
    parameter Boolean normalized=true;

    Modelica_Blocks.Sources.Step step(startTime=0.1, offset=0.1)
        annotation (Placement(transformation(extent={{-60,40},{-40,60}})));
    Modelica_Blocks.Continuous.Filter CriticalDamping(
    ...
    Modelica_Blocks.Continuous.Filter Bessel(
        normalized=normalized,
        analogFilter=Modelica_Blocks.Types.AnalogFilter.Bessel,
        init=init,
        filterType=filterType,
        order=order,
        f_cut=f_cut,
    ...
    Modelica_Blocks.Continuous.Filter Butterworth(
        normalized=normalized,
        analogFilter=Modelica_Blocks.Types.AnalogFilter.Butterworth,
        init=init,
        filterType=filterType,
        order=order,
        f_cut=f_cut,
    ...
    Modelica_Blocks.Continuous.Filter ChebyshevI(
        normalized=normalized,
        analogFilter=Modelica_Blocks.Types.AnalogFilter.ChebyshevI,
        init=init,
        filterType=filterType,
        order=order,
        f_cut=f_cut,
...
<img src=\"modelica://Modelica/Resources/Images/Blocks/Filter1.png\"
  ...
  model FilterWithDifferentiation
    ...
    Modelica_Blocks.Continuous.Der der1
      annotation (Placement(transformation(extent={{-6,40},{14,60}})));
    Modelica_Blocks.Continuous.Der der2
      annotation (Placement(transformation(extent={{30,40},{50,60}})));
    Modelica_Blocks.Continuous.Der der3
...
discontinuous control signal.
  ...
  model FilterWithRiseTime
    ...
    parameter Modelica_SIunits.Time riseTime=2 "Time to reach the step input";
    constant Real pi=Modelica_Constants.pi;
    Modelica_Blocks.Continuous.Filter filter_fac5(f_cut=5/(2*pi*riseTime),
        order=order)
      annotation (Placement(transformation(extent={{-20,-20},{0,0}})));
    Modelica_Blocks.Sources.Step step(startTime=1)
      annotation (Placement(transformation(extent={{-60,20},{-40,40}})));
    Modelica_Blocks.Continuous.Filter filter_fac4(f_cut=4/(2*pi*riseTime),
        order=order)
      annotation (Placement(transformation(extent={{-20,20},{0,40}})));
    Modelica_Blocks.Continuous.Filter filter_fac3(f_cut=3/(2*pi*riseTime),
        order=order)
...
<tr>
  ...
  model InverseModel "Demonstrates the construction of an inverse model"
    ...
    Modelica_Blocks.Continuous.FirstOrder firstOrder1(
      k=1,
      T=0.3,
      initType=Modelica_Blocks.Types.Init.SteadyState)
      annotation (Placement(transformation(extent={{20,20},{0,40}})));
    Modelica_Blocks.Sources.Sine sine(
      freqHz=2,
      offset=1,
      startTime=0.2)
      annotation (Placement(transformation(extent={{-80,20},{-60,40}})));
    Modelica_Blocks.Math.InverseBlockConstraints inverseBlockConstraints
      annotation (Placement(transformation(extent={{-10,20},{30,40}})));
    Modelica_Blocks.Continuous.FirstOrder firstOrder2(
      k=1,
      T=0.3,
      initType=Modelica_Blocks.Types.Init.SteadyState)
      annotation (Placement(transformation(extent={{20,-20},{0,0}})));
    ...
    Modelica_Blocks.Continuous.CriticalDamping criticalDamping(n=1, f=50*sine.freqHz)
...
<img src=\"modelica://Modelica/Resources/Images/Blocks/InverseModel.png\"
  ...
  model ShowLogicalSources
    ...
    extends Modelica_Icons.Example;
    Modelica_Blocks.Sources.BooleanTable table(table={2,4,6,8}) annotation (
        Placement(transformation(extent={{-60,-100},{-40,-80}}, rotation=0)));
    Modelica_Blocks.Sources.BooleanConstant const annotation (Placement(
          transformation(extent={{-60,60},{-40,80}}, rotation=0)));
    Modelica_Blocks.Sources.BooleanStep step(startTime=4) annotation (Placement(
          transformation(extent={{-60,20},{-40,40}}, rotation=0)));
    Modelica_Blocks.Sources.BooleanPulse pulse(period=1.5) annotation (
        Placement(transformation(extent={{-60,-20},{-40,0}}, rotation=0)));

    Modelica_Blocks.Sources.SampleTrigger sample(period=0.5) annotation (
        Placement(transformation(extent={{-60,-60},{-40,-40}}, rotation=0)));
    Modelica_Blocks.Sources.BooleanExpression booleanExpression(y=pulse.y and
          step.y) annotation (Placement(transformation(extent={{20,20},{80,40}},
            rotation=0)));
...
model RealNetwork1 "Demonstrates the usage of blocks from Modelica.Blocks.Math"
  ...
  Modelica_Blocks.Math.MultiSum add(nu=2)
        annotation (Placement(transformation(extent={{-14,64},{-2,76}})));
  Modelica_Blocks.Sources.Sine sine(amplitude=3, freqHz=0.1)
      annotation (Placement(transformation(extent={{-96,60},{-76,80}})));
  Modelica_Blocks.Sources.Step integerStep(height=3, startTime=2)
      annotation (Placement(transformation(extent={{-60,30},{-40,50}})));
  Modelica_Blocks.Sources.Constant integerConstant(k=1)
      annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));
  Modelica_Blocks.Interaction.Show.RealValue showValue
        annotation (Placement(transformation(extent={{66,60},{86,80}})));
  Modelica_Blocks.Math.MultiProduct product(nu=2)
      annotation (Placement(transformation(extent={{6,24},{18,36}})));
  Modelica_Blocks.Interaction.Show.RealValue showValue1(significantDigits=2)
        annotation (Placement(transformation(extent={{64,20},{84,40}})));
  Modelica_Blocks.Sources.BooleanPulse booleanPulse1(period=1)
      annotation (Placement(transformation(extent={{-12,-30},{8,-10}})));
  Modelica_Blocks.Math.MultiSwitch multiSwitch(
      nu=2,
      expr={4,6},
      y_default=2)
      annotation (Placement(transformation(extent={{28,-60},{68,-40}})));
  Modelica_Blocks.Sources.BooleanPulse booleanPulse2(period=2, width=80)
      annotation (Placement(transformation(extent={{-12,-70},{8,-50}})));
  Modelica_Blocks.Interaction.Show.RealValue showValue3(
        use_numberPort=false,
        number=multiSwitch.y,
        significantDigits=1)
        annotation (Placement(transformation(extent={{40,-84},{60,-64}})));
  Modelica_Blocks.Math.LinearDependency linearDependency1(
      y0=1,
      k1=2,
      k2=3) annotation (Placement(transformation(extent={{40,80},{60,100}})));
  Modelica_Blocks.Math.MinMax minMax(nu=2)
...
<li> the Boolean values of the input and output signals are visualized
  ...
  model IntegerNetwork1
    ...
    Modelica_Blocks.MathInteger.Sum sum(nu=3)
      annotation (Placement(transformation(extent={{-14,64},{-2,76}})));
    Modelica_Blocks.Sources.Sine sine(amplitude=3, freqHz=0.1)
      annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
    Modelica_Blocks.Math.RealToInteger realToInteger
      annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
    Modelica_Blocks.Sources.IntegerStep integerStep(height=3, startTime=2)
      annotation (Placement(transformation(extent={{-60,30},{-40,50}})));
    Modelica_Blocks.Sources.IntegerConstant integerConstant(k=1)
      annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));
    Modelica_Blocks.Interaction.Show.IntegerValue showValue
        annotation (Placement(transformation(extent={{40,60},{60,80}})));
    Modelica_Blocks.MathInteger.Product product(nu=2)
      annotation (Placement(transformation(extent={{16,24},{28,36}})));
    Modelica_Blocks.Interaction.Show.IntegerValue showValue1
        annotation (Placement(transformation(extent={{40,20},{60,40}})));
    Modelica_Blocks.MathInteger.TriggeredAdd triggeredAdd(use_reset=false,
        use_set=false)
      annotation (Placement(transformation(extent={{16,-6},{28,6}})));
    Modelica_Blocks.Sources.BooleanPulse booleanPulse1(period=1)
      annotation (Placement(transformation(extent={{-12,-30},{8,-10}})));
    Modelica_Blocks.Interaction.Show.IntegerValue showValue2
        annotation (Placement(transformation(extent={{40,-10},{60,10}})));
    Modelica_Blocks.MathInteger.MultiSwitch multiSwitch1(
      nu=2,
      expr={4,6},
      y_default=2,
    ...
    Modelica_Blocks.Sources.BooleanPulse booleanPulse2(period=2, width=80)
      annotation (Placement(transformation(extent={{-12,-70},{8,-50}})));
    Modelica_Blocks.Interaction.Show.IntegerValue showValue3(use_numberPort=
...
<li> the Boolean values of the input and output signals are visualized
  ...
  model BooleanNetwork1
    ...
    Modelica_Blocks.Interaction.Show.BooleanValue showValue
        annotation (Placement(transformation(extent={{-36,74},{-16,94}})));
    Modelica_Blocks.MathBoolean.And and1(nu=3)
      annotation (Placement(transformation(extent={{-58,64},{-46,76}})));
    Modelica_Blocks.Sources.BooleanPulse booleanPulse1(width=20, period=1)
      annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
    Modelica_Blocks.Sources.BooleanPulse booleanPulse2(period=1, width=80)
      annotation (Placement(transformation(extent={{-100,-4},{-80,16}})));
    Modelica_Blocks.Sources.BooleanStep booleanStep(startTime=1.5)
      annotation (Placement(transformation(extent={{-100,28},{-80,48}})));
    Modelica_Blocks.MathBoolean.Or or1(nu=2)
      annotation (Placement(transformation(extent={{-28,62},{-16,74}})));
    Modelica_Blocks.MathBoolean.Xor xor1(nu=2)
      annotation (Placement(transformation(extent={{-2,60},{10,72}})));
    Modelica_Blocks.Interaction.Show.BooleanValue showValue2
        annotation (Placement(transformation(extent={{-2,74},{18,94}})));
    Modelica_Blocks.Interaction.Show.BooleanValue showValue3
        annotation (Placement(transformation(extent={{24,56},{44,76}})));
    Modelica_Blocks.MathBoolean.Nand nand1(nu=2)
      annotation (Placement(transformation(extent={{22,40},{34,52}})));
    Modelica_Blocks.MathBoolean.Nor or2(nu=2)
      annotation (Placement(transformation(extent={{46,38},{58,50}})));
    Modelica_Blocks.Interaction.Show.BooleanValue showValue4
        annotation (Placement(transformation(extent={{90,34},{110,54}})));
    Modelica_Blocks.MathBoolean.Not nor1
      annotation (Placement(transformation(extent={{68,40},{76,48}})));
    Modelica_Blocks.MathBoolean.OnDelay onDelay(delayTime=1)
      annotation (Placement(transformation(extent={{-56,-94},{-48,-86}})));
    Modelica_Blocks.MathBoolean.RisingEdge rising
      annotation (Placement(transformation(extent={{-56,-15},{-48,-7}})));
    Modelica_Blocks.MathBoolean.MultiSwitch set1(nu=2, expr={false,true})
      annotation (Placement(transformation(extent={{-30,-23},{10,-3}})));
    Modelica_Blocks.MathBoolean.FallingEdge falling
      annotation (Placement(transformation(extent={{-56,-32},{-48,-24}})));
    Modelica_Blocks.Sources.BooleanTable booleanTable(table={2,4,6,6.5,7,9,11})
      annotation (Placement(transformation(extent={{-100,-100},{-80,-80}})));
    Modelica_Blocks.MathBoolean.ChangingEdge changing
      annotation (Placement(transformation(extent={{-56,-59},{-48,-51}})));
    Modelica_Blocks.MathInteger.TriggeredAdd triggeredAdd
      annotation (Placement(transformation(extent={{14,-56},{26,-44}})));
    Modelica_Blocks.Sources.IntegerConstant integerConstant(k=2)
      annotation (Placement(transformation(extent={{-20,-60},{0,-40}})));
    Modelica_Blocks.Interaction.Show.IntegerValue showValue1
        annotation (Placement(transformation(extent={{40,-60},{60,-40}})));
    Modelica_Blocks.Interaction.Show.BooleanValue showValue5
        annotation (Placement(transformation(extent={{24,-23},{44,-3}})));
    Modelica_Blocks.Interaction.Show.BooleanValue showValue6
        annotation (Placement(transformation(extent={{-32,-100},{-12,-80}})));
    Modelica_Blocks.Logical.RSFlipFlop rSFlipFlop
      annotation (Placement(transformation(extent={{70,-90},{90,-70}})));
    Modelica_Blocks.Sources.SampleTrigger sampleTriggerSet(period=0.5,
        startTime=0)
      annotation (Placement(transformation(extent={{40,-76},{54,-62}})));
    Modelica_Blocks.Sources.SampleTrigger sampleTriggerReset(period=0.5,
        startTime=0.3)
...
<li> the Boolean values of the input and output signals are visualized
  ...
  model Interaction1
    ...
    Modelica_Blocks.Interaction.Show.IntegerValue integerValue
      annotation (Placement(transformation(extent={{-40,20},{-20,40}})));
    Modelica_Blocks.Sources.IntegerTable integerTable(table=[0,0; 1,2; 2,4; 3,6;
          4,4; 6,2])
      annotation (Placement(transformation(extent={{-80,20},{-60,40}})));
    Modelica_Blocks.Sources.TimeTable timeTable(table=[0,0; 1,2.1; 2,4.2; 3,6.3;
          4,4.2; 6,2.1; 6,2.1])
      annotation (Placement(transformation(extent={{-80,60},{-60,80}})));
    Modelica_Blocks.Interaction.Show.RealValue realValue
      annotation (Placement(transformation(extent={{-40,60},{-20,80}})));
    Modelica_Blocks.Sources.BooleanTable booleanTable(table={1,2,3,4,5,6,7,8,9})
      annotation (Placement(transformation(extent={{-80,-20},{-60,0}})));
    Modelica_Blocks.Interaction.Show.BooleanValue booleanValue
    ...
    package Interfaces "Interfaces specialised for this example"
      ...
      expandable connector ControlBus
        ...
        import SI = Modelica_SIunits;
          Modelica_SIunits.AngularVelocity realSignal1
          "First Real signal (angular velocity)"   annotation (HideResult=false);
          Modelica_SIunits.Velocity realSignal2 "Second Real signal"
