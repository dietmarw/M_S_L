...
Modelica source.
  ...
  partial package PartialMedium
    ...
    constant Modelica_Media_Interfaces.Choices.IndependentVariables ThermoStates
    ...
    constant Temperature T_default=Modelica_SIunits.Conversions.from_degC(20)
    ...
    final constant Integer nC=size(extraPropertiesNames, 1)
      ...
      annotation (Evaluate=true);
    constant Real C_nominal[nC](min=fill(Modelica_Constants.eps, nC)) = 1.0e-6*
      ones(nC) "Default for the nominal values for the extra properties";
    replaceable record FluidConstants =
        Modelica_Media_Interfaces.Types.Basic.FluidConstants
    ...
    replaceable partial model BaseProperties
      ...
      parameter Boolean standardOrderComponents=true
        ...
        Modelica_SIunits.Conversions.NonSIunits.Temperature_degC T_degC=
            Modelica_SIunits.Conversions.to_degC(T)
        "Temperature of medium in [degC]";
        Modelica_SIunits.Conversions.NonSIunits.Pressure_bar p_bar=
            Modelica_SIunits.Conversions.to_bar(p)
        "Absolute pressure of medium in [bar]";

      // Local connector definition, used for equation balancing check
      connector InputAbsolutePressure = input Modelica_SIunits.AbsolutePressure
        "Pressure as input signal connector";
      connector InputSpecificEnthalpy = input Modelica_SIunits.SpecificEnthalpy
        "Specific enthalpy as input signal connector";
      connector InputMassFraction = input Modelica_SIunits.MassFraction
...
beta is defined as  1/v * der(v,T), with v = 1/d, at constant pressure p.
    ...
    replaceable partial function isothermalCompressibility
      ...
      extends Modelica_Icons.Function;
      input ThermodynamicState state "Thermodynamic state record";
        output Modelica_SIunits.IsothermalCompressibility kappa
...
kappa is defined as - 1/v * der(v,p), with v = 1/d at constant temperature T.
    ...
    type MassFlowRate = Modelica_SIunits.MassFlowRate (
...
<a href=\"modelica://Modelica_Media.UsersGuide\">Modelica_Media.UsersGuide</a>.
  ...
  partial package PartialLinearFluid
    ...
    constant IsobaricExpansionCoefficient beta_const
      ...
      constant Modelica_SIunits.IsothermalCompressibility kappa_const
      "Isothermal compressibility";
    ...
    redeclare function extends setSmoothState
      "Return thermodynamic state so that it smoothly approximates: if x > 0 then state_a else state_b"
    algorithm
        state := ThermodynamicState(p=Modelica_Media.Common.smoothStep(
              x,
              state_a.p,
              state_b.p,
              x_small), T=Modelica_Media.Common.smoothStep(
              x,
              state_a.T,
              state_b.T,
              x_small));
  ...
  partial package PartialMixtureMedium
    ...
    replaceable function gasConstant
      ...
      input ThermodynamicState state "Thermodynamic state";
        output Modelica_SIunits.SpecificHeatCapacity R "Mixture gas constant";
    end gasConstant;

    function moleToMassFractions "Return mass fractions X from mole fractions"
      extends Modelica_Icons.Function;
        input Modelica_SIunits.MoleFraction moleFractions[:]
        "Mole fractions of mixture";
      input MolarMass[:] MMX "Molar masses of components";
        output Modelica_SIunits.MassFraction X[size(moleFractions, 1)]
    ...
    function massToMoleFractions "Return mole fractions from mass fractions X"
      extends Modelica_Icons.Function;
        input Modelica_SIunits.MassFraction X[:] "Mass fractions of mixture";
        input Modelica_SIunits.MolarMass[:] MMX "Molar masses of components";
        output Modelica_SIunits.MoleFraction moleFractions[size(X, 1)]
        "Mole fractions of gas mixture";
    protected
      Real invMMX[size(X, 1)] "Inverses of molar weights";
        Modelica_SIunits.MolarMass Mmix "Molar mass of mixture";
    algorithm
  ...
  partial package PartialTwoPhaseMedium
    ...
    replaceable partial function bubbleEnthalpy
      ...
      input SaturationProperties sat "Saturation property record";
        output Modelica_SIunits.SpecificEnthalpy hl
    ...
    replaceable partial function dewEnthalpy
      ...
      extends Modelica_Icons.Function;
      input SaturationProperties sat "Saturation property record";
        output Modelica_SIunits.SpecificEnthalpy hv
    ...
    replaceable partial function bubbleEntropy
      ...
      extends Modelica_Icons.Function;
      input SaturationProperties sat "Saturation property record";
        output Modelica_SIunits.SpecificEntropy sl
    ...
    replaceable partial function dewEntropy "Return dew point specific entropy"
      extends Modelica_Icons.Function;
      input SaturationProperties sat "Saturation property record";
        output Modelica_SIunits.SpecificEntropy sv "Dew curve specific entropy";
    end dewEntropy;
...
Note that the (small) influence of the pressure term p/d is neglected.
    ...
    redeclare replaceable function setState_psX
      ...
      output ThermodynamicState state "Thermodynamic state record";
    algorithm
      state := ThermodynamicState(p=p, T=Modelica_Math.exp(s/cp_const +
        Modelica_Math.log(reference_T)))
...
This function computes the specific internal energy of the fluid, but neglects the (small) influence of the pressure term p/d.
    ...
    redeclare function extends specificEntropy "Return specific entropy"
      extends Modelica_Icons.Function;
    algorithm
      s := cv_const*Modelica_Math.log(state.T/T0);
    end specificEntropy;
...
quantities are assumed to be constant.
    ...
    algorithm
      state := ThermodynamicState(p=p, T=Modelica_Math.exp(s/cp_const +
        Modelica_Math.log(reference_T) + R_gas*Modelica_Math.log(p/reference_p)));
    end setState_psX;
    ...
    redeclare function extends specificEntropy "Return specific entropy"
      extends Modelica_Icons.Function;
    algorithm
      s := cp_const*Modelica_Math.log(state.T/T0) - R_gas*Modelica_Math.log(
        state.p/reference_p);
    end specificEntropy;
...
Note: Reference enthalpy might have to be extended with enthalpy of formation.
  ...
  package Types "Types to be used in fluid models"
    ...
    type AbsolutePressure = Modelica_SIunits.AbsolutePressure (
        min=0,
        max=1.e8,
        nominal=1.e5,
        start=1.e5)
    ...
    type Density = Modelica_SIunits.Density (
        min=0,
        max=1.e5,
        nominal=1,
        start=1) "Type for density with medium specific attributes";
    type DynamicViscosity = Modelica_SIunits.DynamicViscosity (
        min=0,
        max=1.e8,
        nominal=1.e-3,
        start=1.e-3)
      "Type for dynamic viscosity with medium specific attributes";
    type EnthalpyFlowRate = Modelica_SIunits.EnthalpyFlowRate (
        nominal=1000.0,
        min=-1.0e8,
    ...
    type MolarMass = Modelica_SIunits.MolarMass (
        min=0.001,
        max=0.25,
        nominal=0.032) "Type for molar mass with medium specific attributes";
    type MolarVolume = Modelica_SIunits.MolarVolume (
        min=1e-6,
        max=1.0e6,
        nominal=1.0) "Type for molar volume with medium specific attributes";
    type IsentropicExponent = Modelica_SIunits.RatioOfSpecificHeatCapacities (
        min=1,
        max=500000,
        nominal=1.2,
        start=1.2)
      "Type for isentropic exponent with medium specific attributes";
    type SpecificEnergy = Modelica_SIunits.SpecificEnergy (
        min=-1.0e8,
        max=1.e8,
        nominal=1.e6)
    ...
    type SpecificEnthalpy = Modelica_SIunits.SpecificEnthalpy (
        min=-1.0e10,
        max=1.e10,
        nominal=1.e6)
      "Type for specific enthalpy with medium specific attributes";
    type SpecificEntropy = Modelica_SIunits.SpecificEntropy (
        min=-1.e7,
        max=1.e7,
        nominal=1.e3)
      "Type for specific entropy with medium specific attributes";
    type SpecificHeatCapacity = Modelica_SIunits.SpecificHeatCapacity (
        min=0,
        max=1.e7,
        nominal=1.e3,
        start=1.e3)
      "Type for specific heat capacity with medium specific attributes";
    type SurfaceTension = Modelica_SIunits.SurfaceTension
      "Type for surface tension with medium specific attributes";
    type Temperature = Modelica_SIunits.Temperature (
        min=1,
        max=1.e4,
        nominal=300,
        start=300) "Type for temperature with medium specific attributes";
    type ThermalConductivity = Modelica_SIunits.ThermalConductivity (
        min=0,
        max=500,
        nominal=1,
        start=1)
      "Type for thermal conductivity with medium specific attributes";
    type PrandtlNumber = Modelica_SIunits.PrandtlNumber (
        min=1e-3,
        max=1e5,
        nominal=1.0) "Type for Prandtl number with medium specific attributes";
    type VelocityOfSound = Modelica_SIunits.Velocity (
        min=0,
        max=1.e5,
        nominal=1000,
        start=1000)
    ...
    type DerDensityByPressure = Modelica_SIunits.DerDensityByPressure
      "Type for partial derivative of density with respect to pressure with medium specific attributes";
    type DerDensityByEnthalpy = Modelica_SIunits.DerDensityByEnthalpy
      "Type for partial derivative of density with respect to enthalpy with medium specific attributes";
    type DerEnthalpyByPressure = Modelica_SIunits.DerEnthalpyByPressure
      "Type for partial derivative of enthalpy with respect to pressure with medium specific attributes";
    type DerDensityByTemperature = Modelica_SIunits.DerDensityByTemperature
