within ;
package Modelica_Electrical_StateGraph
  ...
  model ControlledTanks
    ...
    Utilities.TankController tankController
      annotation (Placement(transformation(extent={{-50,-20},{-10,20}},
              rotation=0)));
    Temporary.RadioButton start(reset={stop.on,shut.on}, buttonTimeTable={1,13,
            15,19}) annotation (Placement(transformation(extent={{-90,20},{-70,
                40}}, rotation=0)));
    Temporary.RadioButton stop(reset={start.on,shut.on}, buttonTimeTable={13,15,
            19,21}) annotation (Placement(transformation(extent={{-90,-10},{-70,
                10}}, rotation=0)));
    Temporary.RadioButton shut(reset={start.on,stop.on}, buttonTimeTable={21,
            100}) annotation (Placement(transformation(extent={{-90,-40},{-70,-20}},
              rotation=0)));
...
<li> Button <b>shut</b> is used to shutdown the process,
  ...
  package Utilities "Utility components for the examples"
    ...
    model TankController "Controller for tank system"
      extends Interfaces.PartialStateGraphIcon;
    ...
    model MakeProduct
      ...
      parameter Temporary.SetRealParameter limit=0.98 "Limit level of tank 1"
          annotation (Placement(transformation(extent={{-60,40},{-20,60}},
                rotation=0)));
      parameter Temporary.SetRealParameter waitTime=3 "Wait time" annotation (
            Placement(transformation(extent={{-120,40},{-80,60}}, rotation=0)));
    ...
    model valve
      ...
      Modelica_Blocks.Interfaces.BooleanInput valveControl
        annotation (Placement(transformation(
              ...
              extent={{-20,-20},{20,20}},
              rotation=90)));
      Modelica_Electrical_StateGraph.Examples.Utilities.Inflow2 inflow1
          annotation (Placement(transformation(
              origin={50,0},
              extent={{-50,-50},{50,50}},
              rotation=90)));
      Modelica_Electrical_StateGraph.Examples.Utilities.Outflow2 outflow1
          annotation (Placement(transformation(
              origin={-50,0},
              extent={{-50,-50},{50,50}},
              rotation=90)));
    ...
    model Tank
      ...
      Modelica_Electrical_StateGraph.Examples.Utilities.Inflow1 inflow1
          annotation (Placement(transformation(extent={{-55,60},{-45,70}},
                rotation=0)));
      Modelica_Electrical_StateGraph.Examples.Utilities.Outflow1 outflow1
          annotation (Placement(transformation(extent={{55,-50},{65,-40}},
                rotation=0)));
    ...
    model CompositeStep
      ...
      Alternative Alternative1(nBranches=3) annotation (Placement(
              transformation(extent={{-98,-90},{98,90}},  rotation=0)));
    equation
      connect(transition3.outPort, step3.inPort[1])
