  ...
  results in the following equations:
     ...
     y = u[1] + u[2] + u[3];
      ...
      Modelica_ComplexBlocks_Interfaces.ComplexOutput y annotation (Placement(
        ...
        equation
          y = (if useConjugateInput1 then Modelica_ComplexMath.conj(u1) else u1)
            - (if useConjugateInput1 then Modelica_ComplexMath.conj(u2) else u2);
          annotation (
            Documentation(info="<html>
<p>
      ...
      Modelica_ComplexBlocks_Interfaces.ComplexOutput y
        ...
        equation
          y = k1*(if useConjugateInput1 then Modelica_ComplexMath.conj(u1) else u1)
            + k2*(if useConjugateInput2 then Modelica_ComplexMath.conj(u2) else u2)
            + k3*(if useConjugateInput3 then Modelica_ComplexMath.conj(u3) else u3);
          annotation (
            Documentation(info="<html>
<p>
This blocks computes output <code>y</code> as <i>sum</i> of the
...
<p><b>Example:</b> If <code>useConjugateInput1 = true</code> and <code>useConjugateInput2 = false</code> the output signal <code>y = Modelica.ComplexMath.conj(u1) * u2</code>.</p>
        ...
        equation
          y = (if useConjugateInput1 then Modelica_ComplexMath.conj(u1) else u1)
            / (if useConjugateInput2 then Modelica_ComplexMath.conj(u2) else u2);
          annotation (
            Documentation(info="<html>
<p>
...
<img src=\"modelica://Modelica/Resources/Images/Blocks/Sources/Step.png\"
    ...
    block ComplexRotatingPhasor
      ...
      parameter Modelica_SIunits.AngularVelocity w=1
...
<p>It depends on the implementation of Complex.</p>
               ...
               Polygon(
                 ...
                 fillPattern=FillPattern.Solid,
                 points={{-10.0,0.0},{5.0,5.0},{5.0,-5.0}})}),
    uses(Modelica_Math(version="3.2.2"),
