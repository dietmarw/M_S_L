...
package Modelica_Thermal_HeatTransfer
  ...
  package Examples
    ...
    model TwoMasses "Simple conduction demo"
      ...
      Components.HeatCapacitor mass1(C=15, T(start=373.15, fixed=true))
        annotation (Placement(transformation(extent={{-100,20},{-40,80}},
              rotation=0)));
      Components.HeatCapacitor mass2(C=15, T(start=273.15, fixed=true))
        annotation (Placement(transformation(extent={{40,20},{100,80}},
              rotation=0)));
      Components.ThermalConductor conduction(G=10) annotation (Placement(
            transformation(extent={{-30,-20},{30,40}}, rotation=0)));
      Celsius.TemperatureSensor Tsensor1 annotation (Placement(transformation(
              extent={{-60,-80},{-20,-40}}, rotation=0)));
      Celsius.TemperatureSensor Tsensor2 annotation (Placement(transformation(
              extent={{60,-80},{20,-40}}, rotation=0)));
    equation
...
Tsensor1.T, Tsensor2.T, T_final_degC
    ...
    model ControlledTemperature "Control temperature of a resistor"
      ...
      Modelica_Electrical_Analog.Sources.ConstantVoltage constantVoltage(V=10)
            ...
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Components.HeatCapacitor heatCapacitor(C=1, T(start=TAmb, fixed=true))
      ...
      Modelica_Electrical_Analog.Basic.HeatingResistor heatingResistor(
        ...
        alpha=1/255) annotation (Placement(transformation(
            ...
            extent={{-10,10},{10,-10}},
            rotation=270)));
      Sources.FixedTemperature fixedTemperature(T=TAmb) annotation (Placement(
            transformation(extent={{100,-60},{80,-40}}, rotation=0)));
      Celsius.TemperatureSensor temperatureSensor annotation (Placement(
            transformation(
            origin={10,-30},
            extent={{-10,-10},{10,10}},
            rotation=90)));
      Components.ThermalConductor thermalConductor(G=0.1) annotation (Placement(
...
An appropriate simulating time would be 10 seconds.
    ...
    model Motor "Second order thermal model of a motor"
      ...
      Modelica_Blocks.Sources.CombiTimeTable lossTable(extrapolation=
            ...
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Sources.PrescribedHeatFlow windingLosses(T_ref=368.15, alpha=3.03E-3)
        annotation (Placement(transformation(
            origin={-80,10},
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Components.HeatCapacitor winding(C=2500, T(start=TAmb, fixed=true))
        annotation (Placement(transformation(extent={{-90,-20},{-70,-40}},
              rotation=0)));
      Celsius.TemperatureSensor Twinding annotation (Placement(transformation(
            origin={-60,-50},
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Components.ThermalConductor winding2core(G=10) annotation (Placement(
      ...
      Components.HeatCapacitor core(C=25000, T(start=TAmb, fixed=true))
        annotation (Placement(transformation(extent={{-10,-20},{10,-40}},
              rotation=0)));
      Celsius.TemperatureSensor Tcore annotation (Placement(transformation(
            origin={-20,-50},
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Modelica_Blocks.Sources.Constant convectionConstant(k=25)
        annotation (Placement(transformation(
            ...
            extent={{-10,-10},{10,10}},
            rotation=270)));
      Components.Convection convection annotation (Placement(transformation(
              extent={{30,-20},{50,0}}, rotation=0)));
...
Simulate for 7200 s; plot Twinding.T and Tcore.T.
  ...
  extends Modelica_Icons.Package;
    ...
    model HeatCapacitor "Lumped thermal element storing heat"
      parameter Modelica_SIunits.HeatCapacity C
        "Heat capacity of element (= cp*m)";
      Modelica_SIunits.Temperature T(start=293.15, displayUnit="degC")
...
especially if it shall be allowed that a convective resistance is defined to be zero (i.e. no temperature difference).
    ...
    equation
      Q_flow = Gr*Modelica_Constants.sigma*(port_a.T^4 - port_b.T^4);
      annotation (
        Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{
                100,100}}), graphics={
            Rectangle(
...
as required to keep the temperature at the specified value.
    ...
    model FixedHeatFlow "Fixed heat flow boundary condition"
      parameter Modelica_SIunits.HeatFlowRate Q_flow
    ...
    model ToKelvin "Conversion block from degCelsius to Kelvin"
      ...
      Modelica_Blocks.Interfaces.RealOutput Kelvin(unit="K")
        annotation (Placement(transformation(extent={{100,-10},{120,10}},
              rotation=0)));
    equation
      Kelvin = Modelica_SIunits.Conversions.from_degC(Celsius);
      annotation (
        Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},
                {100,100}}), graphics={
            Ellipse(
    ...
    model FromKelvin "Conversion from Kelvin to degCelsius"
      ...
      Modelica_Blocks.Interfaces.RealOutput Celsius(unit="degC")
        annotation (Placement(transformation(extent={{100,-10},{120,10}},
              rotation=0)));
    equation
      Celsius = Modelica_SIunits.Conversions.to_degC(Kelvin);
      annotation (
        Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{
                100,100}}), graphics={
            Text(
    ...
    model ToKelvin "Conversion block from degFahrenheit to Kelvin"
      ...
      Modelica_Blocks.Interfaces.RealOutput Kelvin(unit="K")
          annotation (Placement(transformation(extent={{100,-10},{120,10}},
              rotation=0)));
    equation
      Kelvin = Modelica_SIunits.Conversions.from_degF(Fahrenheit);
      annotation (
        Diagram(graphics={
            Ellipse(
              extent={{-40,40},{40,-40}},
...
and provides is as output signal.
    ...
    model FromKelvin "Conversion from Kelvin to degFahrenheit"
     ...
     annotation (Placement(transformation(extent={{100,-10},{120,10}}, rotation=
               0)));
    equation
      Fahrenheit = Modelica_SIunits.Conversions.to_degF(Kelvin);
      annotation (
        Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{
                100,100}}), graphics={
            Text(
    ...
    model ToKelvin "Conversion block from degRankine to Kelvin"
      ...
      Modelica_Blocks.Interfaces.RealOutput Kelvin(unit="K")
        annotation (Placement(transformation(extent={{100,-10},{120,10}},
              rotation=0)));
    equation
      Kelvin = Modelica_SIunits.Conversions.from_degRk(Rankine);
      annotation (
        Diagram(graphics={
            Ellipse(
              extent={{-40,40},{40,-40}},
    ...
    model FromKelvin "Conversion from Kelvin to degRankine"
      ...
      Modelica_Blocks.Interfaces.RealOutput Rankine(unit="degRk")
         annotation (Placement(transformation(extent={{100,-10},{120,10}},
              rotation=0)));
    equation
      Rankine = Modelica_SIunits.Conversions.to_degRk(Kelvin);
      annotation (
        Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{
                100,100}}), graphics={
            Text(
