...
function normalize
  ...
  extends Modelica_Icons.Function;
  input Complex v[:] "Vector";
  input Real eps = 100*Modelica_Constants.eps "if |v| < eps then result = v";
     ...
     while i < nv loop
        ...
        if j>=0 then
           ...
           if sortFrequency then
              ...
              if ascending then
                swap := 'abs'(sorted_v[k1]) > 'abs'(sorted_v[k2]);
              else
                swap := 'abs'(sorted_v[k1]) < 'abs'(sorted_v[k2]);
              end if;
           end if;
...
<p>This function returns the inverse Complex tangent of the Complex input.</p>
  ...
  algorithm
    c2 := Complex(Modelica_Math.sinh(c1.re)*Modelica_Math.cos(c1.im),
      Modelica_Math.cosh(c1.re)*Modelica_Math.sin(c1.im));
    annotation(Inline=true, Documentation(info="<html>
<p>This function returns the Complex hyperbolic sine of the Complex input.</p>
  ...
  algorithm
    c2 := Complex(Modelica_Math.cosh(c1.re)*Modelica_Math.cos(c1.im),
      Modelica_Math.sinh(c1.re)*Modelica_Math.sin(c1.im));
    annotation(Inline=true, Documentation(info="<html>
...
<p>This function returns the inverse Complex hyperbolic tangent of the Complex input.</p>
  ...
  algorithm
    c2 := Complex(Modelica_Math.exp(c1.re)*Modelica_Math.cos(c1.im),
      Modelica_Math.exp(c1.re)*Modelica_Math.sin(c1.im));
    annotation(Inline=true, Documentation(info="<html>
<p>This function returns the Complex natural exponential of the Complex input.</p>
  ...
  algorithm
    c2 := Complex(Modelica_Math.log('abs'(c1)), arg(c1));
    annotation(Inline=true, Documentation(info="<html>
...
<p>This function returns the Real absolute of the Complex input, i.e., its length.</p>
  ...
  function arg "Phase angle of complex number"
    extends Modelica_Icons.Function;
    input Complex c "Complex number";
    input Modelica_SIunits.Angle phi0=0
      "Phase angle phi shall be in the range: -pi < phi-phi0 < pi";
    output Modelica_SIunits.Angle phi "= phase angle of c";
  algorithm
    phi := Modelica_Math.atan3(
        c.im,
        c.re,
        phi0);
    annotation(Inline=true, Documentation(info="<html>
...
<p>This function returns the imaginary part of the Complex input.</p>
  ...
  function fromPolar "Complex from polar representation"
    ...
    input Modelica_SIunits.Angle phi "arg of complex";
    output Complex c "= len*cos(phi) + j*len*sin(phi)";
  algorithm
    c := Complex(len*Modelica_Math.cos(phi), len*Modelica_Math.sin(phi));
    annotation(Inline=true, Documentation(info="<html>
<p>This function constructs a Complex number from its length (absolute) and angle (argument).</p>
  ...
  algorithm
    c2 := Complex(sqrt('abs'(c1))*Modelica_Math.cos(arg(c1)/2), sqrt('abs'(c1))
      *Modelica_Math.sin(arg(c1)/2));
    annotation(Inline=true, Documentation(info="<html>
