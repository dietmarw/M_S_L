...
These signals are sampled due to the defined <b>samplePeriod</b> parameter.
  ...
  partial block SVdiscrete "Discrete Single-Variable controller"
    ...
    Modelica_Blocks.Discrete.Sampler sampler_s(final samplePeriod=samplePeriod,
        final startTime=startTime) annotation (Placement(transformation(extent=
              {{-100,-10},{-80,10}}, rotation=0)));
    Modelica_Blocks.Discrete.Sampler sampler_m(final samplePeriod=samplePeriod,
...
to be used as base class for a corresponding controller.
  ...
  partial block MVdiscrete "Discrete Multi-Variable controller"
    ...
    parameter Integer nu_m=1 "Number of measurement inputs";
    parameter Integer ny=1 "Number of actuator outputs";
    Modelica_Blocks.Discrete.Sampler sampler_s[nu_s](each final samplePeriod=
          samplePeriod, each final startTime=startTime) annotation (Placement(
          transformation(extent={{-90,-10},{-70,10}}, rotation=0)));
    Modelica_Blocks.Discrete.Sampler sampler_m[nu_m](each final samplePeriod=
          samplePeriod, each final startTime=startTime) annotation (Placement(
          transformation(
          origin={0,-80},
          extent={{-10,-10},{10,10}},
