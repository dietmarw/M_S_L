...
This library provides simple components for 1-dimensional incompressible thermo-fluid flow models.
  ...
  package Examples
    ...
    model SimpleCooling "Example: simple cooling circuit"
      ...
      parameter Media.Medium medium=Media.Medium() "Cooling medium"
      ...
      Components.HeatedPipe pipe(
        ...
        h_g=0,
        T0fixed=true) annotation (Placement(transformation(extent={{0,-10},{20,
                10}}, rotation=0)));
      Sources.Ambient ambient2(
        constantAmbientTemperature=TAmb,
        medium=medium,
        constantAmbientPressure=0) annotation (Placement(transformation(extent=
...
<td valign=\"top\">10 K</td>
    ...
    model ParallelCooling "Example: cooling circuit with parallel branches"
      ...
      Components.HeatedPipe pipe1(
        ...
        h_g=0,
        V_flow(start=0),
        T0fixed=true) annotation (Placement(transformation(extent={{0,-20},{20,
                0}}, rotation=0)));
...
<td valign=\"top\">15 K</td>
    ...
    model IndirectCooling "Example: indirect cooling circuit"
      extends Modelica_Icons.Example;
      parameter Media.Medium outerMedium=Media.Medium() "Outer medium"
        annotation (choicesAllMatching=true);
      parameter Media.Medium innerMedium=Media.Medium() "Inner medium"
        annotation (choicesAllMatching=true);
      parameter Modelica_SIunits.Temperature TAmb(displayUnit="degC")=293.15
        "Ambient temperature";
      output Modelica_SIunits.TemperatureDifference dTSource=
      ...
      Components.HeatedPipe pipe1(
        ...
        T0fixed=true,
        dpLaminar=1000,
        dpNominal=2000) annotation (Placement(transformation(extent={{20,-50},{
                0,-30}}, rotation=0)));
      Sources.AbsolutePressure absolutePressure(p=10000, medium=innerMedium)
      ...
      Components.HeatedPipe outerPipe(
        ...
        dpNominal(displayUnit="Pa") = 1,
        h_g=0,
        T0fixed=true) annotation (Placement(transformation(extent={{0,60},{20,
                80}}, rotation=0)));
...
<td valign=\"top\">10 K</td>
    ...
    model PumpAndValve "Example: cooling circuit with pump and valve"
      ...
      Components.Valve valve(
        ...
        rho0=10,
        frictionLoss=0) annotation (Placement(transformation(extent={{-30,-10},
                {-10,10}}, rotation=0)));
      Components.HeatedPipe pipe(
        medium=medium,
        T0=TAmb,
